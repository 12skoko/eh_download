...
REPORT_LABELS = {
    "database_only": "仅数据库 completed 中存在",
    "lanraragi_only": "仅 LANraragi 中存在",
    "database_duplicate_ids": "数据库重复 GID",
    "lanraragi_duplicate_ids": "LANraragi 重复 GID",
...
def detail(session, workflow_id):
    return {
        "summary_labels": {
            ...
            "database_resolved_ids": "数据库有效 GID",
            "database_unique_ids": "数据库去重 GID",
            "lanraragi_archives": "LANraragi 档案",
            "lanraragi_resolved_ids": "LANraragi 有效 GID",
            "lanraragi_unique_ids": "LANraragi 去重 GID",
            "database_only": "数据库独有",
    ...
    def run(self):
        ...
        try:
            ...
            result["lanraragi_only_database_states"] = [
                {"manga_id": mid, "status": status, "gid": numeric_database_id(mid)}
