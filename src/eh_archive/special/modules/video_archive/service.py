...
from ...core.events import workflow_event_filter
from ...core.repository import aware
from ...core.service import SpecialConflict, SpecialInvalidRequest, SpecialNotFound
from ...handlers import module_capability
from ...integrations.manga import bind, binding
...
def active_special_workflow(session: Session, manga_id: str) -> SpecialWorkflow | None:
    return session.scalar(
        ...
        .where(
            SpecialWorkflowManga.manga_id == manga_id,
...
class SpecialWorkflowService:
    ...
    def start_for_manga(
        ...
        *,
        ...
        if definition.create is None:
            raise SpecialInvalidRequest("当前特殊处理模块没有创建入口")
        return definition.create(
            self, {"manga_id": manga_id, "row_version": row_version, "load_options": load_options}
        )
    ...
    def _start_video_archive_locked(
        ...
        *,
        ...
        workflow = self.repository.create(
            ...
            actor=self.actor,
            payload=payload,
            resources=[f"manga:{manga.manga_id}"],
            initialize=lambda workflow: bind(
                self.session, workflow, manga, entry=entry, resume_status=manga.status
            ),
            emit_event=False,
        )
        workflow.progress = {"message": VIDEO_ARCHIVE.initial_phase}
    ...
    def dispatch_source_cleanups(self) -> BatchDispatchResult:
        ...
        rows = list(
            self.session.execute(
                ...
                .join(SpecialWorkflowManga, SpecialWorkflowManga.workflow_id == SpecialWorkflow.id)
                .join(MangaRecord, MangaRecord.manga_id == SpecialWorkflowManga.manga_id)
                .where(
                    SpecialWorkflow.kind == VIDEO_ARCHIVE.kind,
    ...
    def cancel(self, workflow_id: int, *, row_version: int) -> SpecialJob:
        ...
        if running is not None:
            if not workflow.cancel_requested_at:
                now = utcnow()
    ...
    def release_expired_job(
        ...
        *,
        ...
        manga = self.session.scalar(
            select(MangaRecord)
            .where(MangaRecord.manga_id == binding(workflow).manga_id)
            .with_for_update()
        )
        if manga is None:
    ...
    def exit_without_cleanup(
        ...
        *,
        ...
        workflow.row_version += 1
        previous = manga.status
        manga.status = binding(workflow).resume_status or Status.MANUAL_REVIEW.value
        manga.status_updated_at = manga.updated_at = now
...
def special_workflow_detail(session: Session, workflow_id: int) -> dict[str, Any]:
    ...
    if workflow is None:
        raise SpecialNotFound("特殊工作流不存在")
    manga = session.get(MangaRecord, binding(workflow).manga_id)
    jobs = list(
        session.scalars(
            select(SpecialJob)
    ...
    candidate_events = list(
        session.scalars(
            select(EventLog)
            .where(
                workflow_event_filter(session, workflow.id),
    ...
    expired_job = (
        running
        if running and running.lease_until is not None and aware(running.lease_until) < utcnow()
        else None
    )
    payload = dict(workflow.payload or {})
...
def list_video_workflows(session: Session, *, limit: int = 200, page: int = 1) -> dict[str, Any]:
    rows = list(
        session.scalars(
            select(SpecialWorkflow)
            .where(SpecialWorkflow.kind == VIDEO_ARCHIVE.kind)
            .order_by(desc(SpecialWorkflow.updated_at), desc(SpecialWorkflow.id))
            .offset((max(1, page) - 1) * max(1, min(limit, 500)))
    ...
    manga_statuses = {
        ...
        for manga in session.scalars(
            select(MangaRecord).where(
                MangaRecord.manga_id.in_([binding(row).manga_id for row in rows])
            )
        )
    }
    cleanup_pending = 0
    ...
    for workflow in rows:
        ...
        if cleanup.get("status") not in {"pending", "failed"}:
            continue
        if manga_statuses.get(binding(workflow).manga_id) == Status.COMPLETED.value:
            cleanup_pending += 1
        else:
            cleanup_waiting += 1
    return {
