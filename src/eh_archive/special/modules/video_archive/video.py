...
class VideoArchiveExecutor:
    def __init__(
        ...
        *,
        ...
        with database.session() as session:
            ...
            b = binding(workflow)
            self.manga_id = b.manga_id
            self.artifact_generation = b.context.get("expected_artifact_generation")
    ...
    def _state(self) -> tuple[MangaRecord, SpecialWorkflow, dict[str, Any]]:
        with self.database.session() as session:
            values = SpecialRepository(session, timezone=self.app.timezone).validate_video_claim(
                self.claim.job_id,
    ...
    def _special_paths(self, manga_id: str, role: str) -> tuple[str, Path, str]:
        ...
        separator = "\\" if "\\" in external_root and "/" not in external_root else "/"
    ...
    def _compose(
        ...
        generation = (self.artifact_generation or 0) + 1
        ...
        def before_promote() -> None:
            with self.database.session() as session:
                values = SpecialRepository(session).validate_video_claim(
                    self.claim.job_id,
                    workflow_id=self.claim.workflow_id,
                    lease_token=self.claim.lease_token,
                    lease_owner=self.claim.lease_owner,
                )
                if values is None or values[2].artifact_generation != self.artifact_generation:
                    raise ArchiveError(
                        "stale_special_job", "final artifact fencing failed", ErrorClass.TEMPORARY
                    )
                if values[1].cancel_requested_at:
                    raise SpecialCancellationRequested
    ...
    def _cleanup_qbit(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for item in payload.get("torrents", []):
            ...
            if torrent_category(info) != self.module.download.category:
                skipped.append(role)
                continue
            expected_path, _, _ = self._special_paths(self.manga_id, role)
    ...
    def cancel_video_archive(self) -> None:
        ...
        numeric_id = safe_filename(self.manga_id.split("/", 1)[0])
