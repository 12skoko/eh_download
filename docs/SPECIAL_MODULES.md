...
再次加载会清除之前的选择。手动候选分为普通、过时两段，所有候选均可选择；过时和重采样标红，无 Seeder 标黄，不阻止提交。视频和过小候选也不再确认告警。
提交前重新取得当前候选，候选身份变化时重新选择；无 Seeder 仍按用户选择提交 qBittorrent，不自动转直接下载。普通自动选种的排除与回退规则不变。

手动下载优先使用 `href`。勾选备用链接选项后，href 请求失败、返回不存在或无效种子内容时，尝试合法的同种子 `onclick` 地址；未勾选则只使用 href。选择与勾选结果一起保存，重试沿用，更换选择时以新提交的勾选为准。普通自动下载仍只在明确返回种子不存在时、经档案详情页授权后尝试备用链接。
...
旧 `invalid_torrent` 等没有告警快照的记录，可在错误框下方勾选允许尝试带账号信息的备用链接，再点击“确认选择”。
保存不会重新排队，仍使用现有“等待下载 / torrent”操作开始下载。
该授权只交给下一次自动选种尝试，并绑定当次选中的候选；仅在普通链接明确返回不存在时使用，其他错误不放行。
