...
Supervisor 会按需运行 `screen`、`details`、`torrent_download`、`direct_download`、`validate`、`prepare`、`upload`、`cleanup` 和 `delete`。首次选择种子前必须取得完整 MangaInfo。程序忽略 `Outdated Torrents` 和红色时间的过时种子以及明确的 `1280x/800x/1920x/2560x` 重采样；仅剩这些种子时根据 `fallback_method` 切换 direct/H@H/aria2，配置为 `none` 时则进入 `download_blocked`。非过时种子中出现视频标记时进入 `manual_review`，即使它同时是重采样；确认视频告警后把视频种子当作普通种子，也兼容 remark 中的 `skip video`。小于预计大小 60% 的种子视为异常。其余候选用“同时更大且更新”淘汰旧版本；胜出版本没有 Seeder 时转 `direct` 下载；不同大小版本无法比较时进入 `manual_review`；剩余候选大小相同时依次按 Seeder 数和发布时间选择。

普通下载的“视频”和“种子过小”告警支持在详情页集中确认并重新排队，授权保存在结构化字段中，候选或预计大小变化后重新确认。普通链接明确返回种子不存在时，可授权尝试同一种子的个性化链接；其他无效响应不触发换链接。手动选种及异常恢复详见 [特殊模块说明](SPECIAL_MODULES.md#手动种子下载与普通告警放行)。
...
同名档案确实属于新旧版本时，选择“以当前档案为最新版”：勾选要淘汰的旧档案，并选择当前档案进入“等待上传”或“已下载，重新校验”。提交必须填写原因并确认；服务端在同一事务内校验全部选中记录的版本和租约，旧档案设为 `outdated`，其 `superseded_by_id` 指向当前档案。未勾选的记录不变，已有其他替代关系或活动任务的档案不能选中。

上传调度统一等待所有 `superseded_by_id` 指向新档案的旧档案变为 `deleted`，包括已有的自动版本替代关系。旧档案删除失败并进入人工复核时仍会阻止新档案上传；详情页会列出等待的旧档案及其状态。delete 模块暂停或未运行时，新档案继续等待，但不阻止其他无依赖档案上传。普通删除会保留与替代档案共用的本地路径，发现新旧档案共享 LANraragi ID 则转人工复核。现有删除状态门槛保持不变，替代档案从 `download_pending` 起即可允许删除旧档案。

部署此功能时运行 `eharchive --config-dir config db upgrade`，迁移 `0018_superseded_upload_wait` 为替代关系查询增加索引，然后重启 Web 和任务进程以加载新逻辑。
...
历史 cleanup/delete 异常造成下载目录遗留时，可以运行独立维护脚本。脚本默认只预览，扫描 `eharchive` 分类且名称为纯数字 ID 的 qBittorrent 任务，以及 torrent、direct、H@H、aria2 四个下载目录；只有数据库状态为 `completed` 或 `deleted` 且没有活动 attempt/租约的项目才会进入删除计划。它不修改数据库，也不调用 LANraragi。确认预览报告后再执行全量清理：
...
总览的 `collect` 和 `torrent_check` 下方显示“下次计划启动”，按配置时区每三秒自动刷新。时间由 Supervisor 写入 `system_control`，仅在其心跳、租约有效，且模块的 owner 与当前 Supervisor 相同时显示。定时调度仍使用 Supervisor 内存中的单调时钟；重启后按初始延迟重新计时，不沿用旧计划。

点击“立即执行”会先打开确认弹窗，取消不会提交请求。确认后页面依次反馈请求提交、启动结果；“已启动”不表示任务已经完成。手动运行仍遵守暂停、禁用、维护和冷却限制，运行中或已有待处理请求时不能重复提交。手动启动会重新计算下次计划时间。尚未处理的请求绑定提交时的 Supervisor owner，重启后失效，需要重新确认提交。

此功能需要迁移 `0020_module_schedule`。升级时先停止 Web 和 Supervisor，运行 `eharchive --config-dir config db upgrade`，再启动两者。
...
Web 顶部导航的“日志”（`/logs`）以文件夹方式浏览 `log_dir`：每页仅显示当前目录的直接子文件夹及 `.log`、`.jsonl` 文件，点击文件夹可逐层进入。顶部路径导航支持跳回任意上级，也可点击“返回上一级”；查看日志后可返回所在目录。文件夹优先显示，日志按更新时间倒序，每页 100 项，名称筛选仅作用于当前目录。每次最多检查当前目录的 20,000 个条目，超过时页面会提示结果不完整；链接文件和链接目录不会列出。

打开文件后默认显示末尾最多 64 KiB 原始文本。“更早内容”切换到前一个窗口，“最新内容”返回文件末尾。自动刷新默认关闭，可开启每 5 秒刷新；切换到后台标签页会暂停轮询。每次重新读取尾部，因此任务报告原地更新的进度行也能显示。窗口边界可能截断一行；完整内容请使用“下载完整日志”。

单文件下载直接传输原始文件，不做脱敏，复用 Web 登录权限；下载长度以开始时的文件大小为上限。正在写入或改写的文件不保证事务快照一致性。此功能不会修改日志、数据库或任务状态。

“打包下载整个日志目录（ZIP）”会打包 `log_dir` 下所有普通文件（不限扩展名）和子目录，保留相对路径与空目录，跳过符号链接和链接目录；不受当前浏览目录、名称筛选或列表扫描上限影响。压缩包在系统临时目录中生成，完成后分块下载，结束后清理临时文件；需要足够的临时磁盘空间。每个 Web 进程同时只生成一个压缩包。每个源文件按打开时的长度读取，运行中的日志不保证跨文件快照一致性。
