...
- MangaInfo 获取是可提前执行的辅助操作，不是 torrent 的前置条件。详情获取失败不能阻塞 torrent；进入 direct/H@H 或上传前必须成功执行对应的详情检查。
- LANraragi 使用 Archive API：上传成功响应中的 40 位 SHA1 archive ID 才能标记成功；上传结果不确定时，允许按当前产物 SHA1 查询已知 archive ID，但不能凭文件名或模糊标题猜测成功。
...
discovered
   ...
   `-- 筛选通过 -> download_pending
         `-- 尝试获取 MangaInfo（失败只记录，不阻塞 torrent）
...
- MangaInfo 获取失败只记录 `collect_details` 操作的错误和重试时间，不阻塞 torrent 提交；torrent 只依赖列表记录和有效 torrent 信息。
- `download_pending`、`downloading`、`downloaded` 和 `validating` 可以暂时没有 MangaInfo，但 `upload_pending` 和 `uploading` 必须存在可上传的 MangaInfo。
...
- 正式文件名包含安全化 manga_id 和 artifact generation；每个 generation 使用新文件名，不覆盖上一 generation。
...
- 默认在替代档案已经 `uploaded` 或 `completed` 后删除旧 LANraragi 档案，避免新档案失败时出现远端空档。
...
每次重新下载、从目录生成 ZIP、移动到 quarantine 或人工替换文件时都递增 `artifact_generation`。正式文件名至少包含安全化 manga_id 和 generation，例如 `<safe_manga_id>.g2.zip`；临时文件名还包含 attempt ID，例如 `<safe_manga_id>.g2.a123.tmp`。安全化规则必须确定、跨平台一致并检测截断后的名称冲突。
...
- `downloading + direct/aria2`：检查临时文件后回到 `download_pending` 或继续下载。
- `downloading + torrent`：根据 qBittorrent hash 查询实际状态。
- `validating`：回到 `downloaded`。
- `preparing`：删除或检查临时压缩文件后重试。
- `uploading`：使用 attempt 记录的 artifact SHA1 调用 `GET /api/archives/{sha1}/metadata`；确认存在则进入 `uploaded`，无法确认则进入 `manual_review`，禁止直接盲目重传。
- `uploaded` 清理租约过期：继续幂等清理。
- `outdated` 删除租约过期：使用已持久化的 `lrr_archive_id` 重新查询 LANraragi 和本地状态。

### 6.7 状态迁移入口

所有状态修改通过统一的状态迁移服务执行，不允许采集器、下载器、Web 路由各自任意赋值。
...
| `outdated` | 旧档案远端和本地删除完成 | 替代档案为 uploaded/completed，DELETE success=1、metadata 返回 400，且本地遗留项已安全清理 | `deleted` | 保留业务、attempt 和删除事件历史 |
| `outdated` | 删除结果不确定 | 替代档案未就绪、ID 非法、423 或响应冲突 | `manual_review` | 禁止猜测删除完成 |
...
| `manual_review` | 人工选择安全恢复点 | 目标状态全部前置条件满足 | `download_pending/validating/upload_pending/outdated` | 记录 actor、理由和证据 |
...
所有下载器和准备任务必须遵守基于安全化 manga_id、artifact generation 和 attempt ID 的确定性命名规则。正式文件名包含 generation，临时文件名额外包含 attempt ID；数据库只登记当前正式产物的 location 和文件名，不登记临时文件。旧文件迁移时先扫描各配置根目录，再将能够唯一匹配 manga_id、旧 filename、torrent hash 或 SHA1 的结果回填；无法唯一匹配的记录进入 `manual_review`，不能凭猜测删除或上传。Supervisor 重启后只依赖配置根目录、location 和文件名重新定位，不依赖旧进程内存。
...
| `remark` | `text` | 人工备注；程序不得解析该字段控制流程 | `manga.remark` |
...
| `artifact_filename` | `text` | 当前 generation 的正式产物文件名；新产物必须包含安全化 manga_id 和 generation | `manga.filename` |
...
- 上传 attempt 的 `external_effect_started_at` 已存在但没有明确成功响应时，恢复流程必须先按 `artifact_sha1` 查询 LRR；无法确认则人工审核。
...
| `operation` | `varchar(16)` nullable | 当前业务操作 |
...
| 字段 | PostgreSQL 类型 | 说明 |
|---|---|---|
| `component` | `varchar(32)` PK | all/collect/torrent_download/direct_download/validate/prepare/upload/cleanup/delete |
| `state` | `varchar(16)` | running/paused |
...
`all=paused` 阻止所有新任务；组件暂停只阻止相应任务，不中断无关任务。`supervisor` scope 使用数据库租约保证同一部署只有一个调度者，第二个 Supervisor 在已有未过期租约时拒绝启动；collect/reconcile 等周期任务也使用独立 scope 防止重复 tick。

### 8.7 索引与约束

至少建立：

- `manga(status, priority DESC, next_retry_at, created_at)`：Supervisor 领取任务。
- `manga(lease_until)` 的部分索引：回收过期租约。
- `manga(lrr_archive_id)`：远端核对。
- `manga(external_download_id)`：qBittorrent/aria2/H@H 核对。
- `manga(superseded_by_id)`：新旧关系。
- `job_attempt(manga_id, started_at DESC)`：档案 attempt 时间线。
- `job_attempt(operation, status, started_at)`：运行、失败和 abandoned attempt 查询。
- `event_log(manga_id, created_at DESC)`：档案历史。
- `event_log(component, created_at DESC)`：系统日志。
...
- `manga.remark` 原样迁移到新 `remark`，作为可由 Web 编辑的人工备注。旧值即使包含历史控制文本也只作为文本保留，新程序禁止根据 remark 内容改变状态或执行动作。
...
- 创建 manga、mangainfo、job_attempt、event_log、system_control 表，并添加 active attempt 的同 manga 复合外键。
...
- 项目中不存在 SQLite、Redis、Celery 或硬编码平台路径。
...
- 筛选通过后先进入 download_pending，并尽早使用 browse_session 幂等 upsert MangaInfo；获取失败只记录 details 错误和退避，不阻塞 torrent。
...
- MangaInfo 暂时不可用时 torrent 仍可提交和完成；direct/H@H/aria2 与上传在详情完整前被正确阻塞。
...
- 实现 download_pending/downloading/downloaded 状态和租约恢复。
...
- 实现流式 LANraragi 上传以及明确的 200/409/400/415/417/422/423/500 分类处理。
- 实现上传结果未知时的远端核对。
...
- 实现状态总览、队列、错误、隔离和事件页面。
- 实现档案详情和状态历史。
- 实现人工备注查看和编辑；修改写入 event_log，但 remark 本身只保留当前内容。
- 实现手工 URL、priority、重试、跳过、恢复和取消。
- 实现组件暂停/恢复。
- 实现 Supervisor/qBittorrent/LANraragi/存储健康页面。
- 加入认证、权限限制和操作审计。
